...
const HandleDelete=(id)=>{
  const cart=  data.filter((value)=>(value.id!==id))
         ...
         .then(axios.post('http://localhost:3000/users',{...del,Cart:cart}))
  ...
  return (
    <>
            ...
            <div className="hero bg-base-200" key={index} >
              <div className="hero-content flex-col lg:flex-row">
                ...
                <div>
                  ...
                  <h1>Price : {value.price * value.count}</h1>
      ...
      <div className='subtotal-main bg-base-200'>
        <div className='subtotal-sub'><h1 className="text-3xl font-bold">Subtotal ({data&&data.length} items): { }</h1></div>
      </div>
      <div className='subtotal-main bg-base-200'>
        {data&&data.length >= 1 ?
          <div className='subtotal-sub' ><button className="btn btn-primary">Proceed to buy</button></div> : "add to cart"
